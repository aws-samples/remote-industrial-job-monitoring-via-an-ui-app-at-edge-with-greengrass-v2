...
_setEnv()
{
  ...
  DEV="dev"
  PRD="prd"
  DEV_IOT_THING_GROUP="REPLACE-WITH-YOUR-THING-GROUP"
  AWS_ACCOUNT_NUMBER="REPLACE-WITH-YOUR-OWN-ACCOUNT-NO"
  AWS_REGION="REPLACE-WITH-YOUR-OWN-REGION"
  ROLE_ARN="REPLACE-WITH-YOUR-OWN-ROLE"
  NEXT_VERSION="1.0.0"
  COMPONENT_NAME="com.websocketApp"
  ...
  SRC_FOLDER="src"
  ARTIFACTS_ARCHIVE_FILE_NAME="com.websocketApp"
  MAIN_ARTIFACT_FILE="app.py"
...
_getCurrentVersion()
{
    ...
    echo "Checking and reading the version file exists in the S3Path : ${S3_BUCKET}/${S3_KEY}/${COMPONENT_NAME}/${CURRENT_VERSION_FILE}"
    not_exist=true
    aws s3api head-object --bucket ${S3_BUCKET} --key ${S3_KEY}/${COMPONENT_NAME}/${CURRENT_VERSION_FILE}
    if [ $? == 0 ]; then
      not_exist=false
    fi
    echo $not_exist
...
_prepare_deployment_config_file_based_on_deployment_name()
{
  ...
  if [[ -z "${deployment_id}" ]]; then
    ...
    STR4=${NEXT_VERSION}
    STR5=',"configurationUpdate":{"reset":[""]}}}'
    ...
    #echo $NEXT_VERSION
    NEW_CONFIG_JSON=`aws greengrassv2 get-deployment --deployment-id ${deployment_id} | jq .'components' | jq 'del(."$COMP_NAME")' | jq  '. += {"'"$COMP_NAME"'": {"componentVersion": "'"$NEXT_VERSION"'","configurationUpdate":{"reset":[""]}}}'`
  fi
...
_deploy_configuration_on_devices()
{
  ...
  if [ $? -ne 0 ]
    then
    echo -e "date : Error occurs while deploying "
    updateS3Flag=false
  fi
}
...
if [ $updateS3Flag == true ]; then
  _updateTheVersionInFileInS3
else
  echo "Not updating the S3 file"
fi